...
search = cPickle.load(open(data_dir + 'pulled_pork_search.pickle', 'rb'))
...
for i in range(ax.shape[0]):
    for j in range(ax.shape[1]):
        ax[i, j].set_xlim(0, 1)
plt.savefig(plot_dir + 'pulled_pork_flavors.png')
plt.close()
...
plt.savefig(plot_dir + 'pulled_pork_ingred_counts.png')
plt.close()
...
plt.tight_layout()
plt.savefig(plot_dir + 'pulled_pork_ingred_counts_top.png')
plt.close()